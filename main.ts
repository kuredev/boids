...
class Flock {
  ...
  public avoidWall(boid: Boid): Boid {
    ...
    // それまでのRad計算は全部無視する
    if((this.closeXRight(boid) && this.faceX(boid)) || (boid.coordinate.x > WALLX && this.faceX(boid))){
      this.inversionX(boid);
    } else if((this.closeXLeft(boid) && this.faceX0(boid)) || (boid.coordinate.x < 0 && this.faceX0(boid))){
      this.inversionX(boid);
    } else if((this.closeYTop(boid) && this.faceYTop(boid)) || (boid.coordinate.y > 0 && this.faceYTop(boid))){
      this.inversionY(boid);
    }else if((this.closeYButtom(boid) && this.faceYButtom(boid)) || (boid.y() * -1 > WALLY && this.faceYButtom(boid))){
      this.inversionY(boid);
    }
  ...
  public faceYButtom(boid: Boid): boolean{
    let rad = this.calcAverageRads(boid.drads);
    if ((rad < 6.28319 && rad > 3.14159) || (rad > -3.14159 && rad < -0.0174533)){
      return true;
    }else{
      return false;
    }
  ...
  public updateCoordinateAndRad(boid: Boid){
    ...
    let updatedCoordinate = this.addCordinateToRad(boid.coordinate, aveDradsUpteaed);
    boid.drads = [];
  ...
  public addCordinateToRad(c: Coordinate, rad: number): Coordinate{
    return { x: c.x + (Math.cos(rad) * MOVE_FACTOR), y: c.y + (Math.sin(rad) * MOVE_FACTOR) }
  }
  ...
  public avoidOtherBoids(boid: Boid){
    ...
    for (let otherBoid of boids) {
      let closeDistance: boolean = this.closeDistanceCoordinate(boid.coordinate, otherBoid.coordinate, CLOSE_BOID_DISTANCE);
      if(closeDistance) {
        let vector: Vector = this.coordinatesToVector(boid.coordinate, otherBoid.coordinate);
  ...
  public closeDistanceCoordinate(c1: Coordinate, c2: Coordinate, standardDistance: number): boolean{
    let distance: number = Math.sqrt(Math.pow(c1.x - c2.x, 2) + Math.pow(c1.y - c2.y, 2));
